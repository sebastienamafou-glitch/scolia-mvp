...
import { Logo } from './Logo'; // Ton composant logo existant
...
interface Student {
  ...
  schoolName: string; // On passera le nom de l'école
}

export const DigitalIdCard: React.FC<{ student: Student }> = ({ student }) => {
  
  // Données contenues dans le QR Code (ex: JSON avec ID et sécurité)
  ...
  return (
    <div style={{
        ...
        height: '220px',
        borderRadius: '15px',
        background: 'linear-gradient(135deg, #0A2240 0%, #005f99 100%)', // Bleu Scolia
        color: 'white',
        padding: '20px',
        ...
        {/* PIED DE CARTE : QR Code + Matricule */}
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'end', zIndex: 2 }}>
            <div style={{ fontSize: '0.7rem', opacity: 0.7 }}>
                ID: {String(student.id).padStart(6, '0')} <br/>
                Né(e) le : {student.dateNaissance ? new Date(student.dateNaissance).toLocaleDateString() : 'N/A'}
            </div>
